describe 'OLA - Student Loan Products', loan_type: 'dental_residency', loan_type_code: 'MXDL', page_type: 'form', order: :defined do
  ...
  describe "Dental Residency and Relocation Form" do
    it "exists for following tests to use, otherwise they are skipped", :smoke do
      goto_page_ola loans.loan_type_id.dental_residency_and_relocation_loan
      expect(find p.globals.main_form).to be
    end
  end

  describe "Dental Residency and Relocation Form - Smoke Saved" do
    it "is able to continue from a previously saved application", :smoke, :api do
      ssn = DynamoActions.increment_ssn
      api_response = create_loan ssn, loans.loan_type.dental_residency_and_relocation
  ...
  describe "Dental Residency and Relocation General Information Page - First Name Left Blank" do
    it "has a general information form for dental residency and relocation loans that is filled out incorrectly", :sad do
      goto_page_ola loans.loan_type_id.dental_residency_and_relocation_loan
      ssn = DynamoActions.increment_ssn
  ...
  describe "Dental Residency and Relocation School Information Page  - Degree Left Blank" do
    it "has a school information form for dental residency and relocation loans that is filled out incorrectly", :sad, :api do
      create_loan_using_api_and_continue_to_school_info p, loans.loan_type.dental_residency_and_relocation
      ...
      expect(find_by_id(p.school_information.major).value).to eq 'D05'
  ...
  describe "Dental Residency and Relocation Loan Happy All Pages" do
    it "has a form for dental residency and relocation student loans that is filled out correctly", :happy do
      goto_page_ola loans.loan_type_id.dental_residency_and_relocation_loan
      ssn = DynamoActions.increment_ssn
      fill_out_basic_information_form p, ssn
      confirm_date_of_birth p
